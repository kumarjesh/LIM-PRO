...
// Uses the Open Food Facts API (completely free, no key needed)
async function fetchByBarcode(barcode) {
  ...
  // Build a fake "image" — a canvas with the nutrition text so it flows into our existing pipeline
  ...
  lines.forEach((l, i) => ctx.fillText(l, 20, 80 + i * 24));
    ...
    try {
      ...
      scanner.start(
        ...
        async (code) => {
          scanner.stop();
          setStatus("loading");
          setMsg(`Found barcode: ${code} — fetching product...`);
          try {
            const imageB64 = await fetchByBarcode(code);
            onResult(imageB64);
          } catch {
            setStatus("error");
            setMsg("Product not found in Open Food Facts database. Try scanning the label directly.");
          }
        },
        () => {} // ignore frame errors
      );
      ...
      setStatus("error");
      setMsg("Could not start barcode scanner. Try the camera or gallery mode.");
    }
  };
    ...
    <div style={{ marginBottom: 16 }}>
      <div
        ...
        style={{
          ...
          background: "#0d1117",
          minHeight: status === "scanning" ? 220 : 0,
        }}
      />
      ...
      {status === "scanning" && (
        <button className="btn btn-secondary" style={{ marginTop: 10 }} onClick={stopScan}>
          ✕ STOP SCANNING
        </button>
      )}
